...
export default function CambiarEstadoPedidoPage() {
  ...
  useEffect(() => {
    console.log('🔄 CambiarEstadoPedidoPage - Cargando pedido con ID:', id);
    async function cargar() {
      try {
        setLoading(true);
        console.log('📡 Solicitando pedido...');
  ...
  async function guardar(e) {
    ...
    // Validar que tengamos un ID válido antes de intentar actualizar
    if (!id || isNaN(+id)) {
    ...
    try {
      ...
      navigate("/pedidos", { replace: true, state: { reload: Date.now() } });
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="page" style={{ textAlign: 'center', padding: '50px' }}>
        <p style={{ fontSize: '18px', color: '#666' }}>Cargando pedido...</p>
      </div>
    );
  }
